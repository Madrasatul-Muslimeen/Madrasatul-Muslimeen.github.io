# QuranRevival — build log

Every round from the 9 August 2026 cutover onward, oldest first. **This file is
history, not the brief** — `CLAUDE.md` is what every session reads. Come here
only when you need the background of one particular feature: why a decision was
taken, what a round measured, what it deliberately left undone.

The five most recent rounds are also kept in `CLAUDE.md`, so recent context
needs no trip here.

Split out of `CLAUDE.md` on 4 September 2026 (v07.126), on the owner's own
instruction — the brief had grown to 689 KB, roughly 172,000 tokens, and was
read in full at the start of every session before a word of the actual task.
Nothing was edited on the way across: the entries below are byte-for-byte what
`CLAUDE.md` carried, and the standing lessons buried in their prose were
lifted into a new "Standing lessons" section of the brief rather than left
only here.

---

Cutover to production happened
