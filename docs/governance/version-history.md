# QuranRevival version history

Moved out of `app/js/version.js` on 25 Sep 2026 (issue #272, load speed) --
that file was 40KB and nearly all of it was this history, sitting in a
module every page imports before first paint. `app/js/version.js` now
holds only the `APP_VERSION` constant and a two-line pointer here.

Everything below is moved word for word from the comments that used to sit
above `export const APP_VERSION` in that file, oldest first, unedited --
this is a historical record, not rewritten to read differently now.
