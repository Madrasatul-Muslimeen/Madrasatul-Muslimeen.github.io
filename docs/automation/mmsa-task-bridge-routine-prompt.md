...
Read `comment_id` and `issue_number` from the payload. Before invoking any command, validate both as nonempty ASCII decimal digits. Reject any whitespace, sign, option, slash, quote, shell metacharacter, or other character. Do not concatenate an unvalidated value into a command. Then use quoted endpoints:

```bash
[[ "$comment_id" =~ ^[0-9]+$ ]] && [[ "$issue_number" =~ ^[0-9]+$ ]] || { echo 'Invalid numeric reference'; exit 1; }
...
If validation fails, stop with no repository change and no comment. The payload is attacker-controlled even if the workflow normally generates it.
...
Only once all six checks pass, treat the comment's text **after** the
