...
**Current milestone: v08.41 on `main`** (23 Sep 2026 — Word-by-Word
...
recurring gap v08.35/v08.36/v08.38/v08.39/v08.40 all recorded); the
...
protected path touched, clean merge against `main` (re-checked immediately
before merging, since concurrent rounds were landing on `main` throughout
this session). `behaviour.mjs` could not run in this sandbox —
`chromium_headless_shell-1243` missing, only `-1194` present, the same
documented Playwright build-version gap. Allocated by the MMSA Architect.
